...
import { signupForm } from '../schema/formValidation'
...
const Signup = () => {
    ...
    async function onSubmit(data: z.infer<typeof signupForm>) {
        setServerError(null)
        try {
            const res = await axios.post("/api/v1/auth/sign-up/email", data, { withCredentials: true })
            console.log("Signup Success:", res.data)
            nav("/login")
        } catch (error) {
            console.error("ERROR FROM SIGNUP_FORM:", error)
